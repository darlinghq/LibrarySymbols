// Taken from https://github.com/keith/dyld-shared-cache-extractor/blob/main/dyld-shared-cache-extractor.c
...
extern "C" 
int (*extract)(const char *cache_path, const char *output_path, void (^progress)(int, int)) = nullptr;

int get_library_path(const char *candidate, char *output) {
	if (candidate) {
...
int extract_shared_cache(const char *library_path, const char *cache_path, const char *output_path) {
	...
	*(void **)(&extract) = dlsym(handle, "dyld_shared_cache_extract_dylibs_progress");
	...
	int result = extract(cache_path, output_path, ^void(int completed, int total) {
		printf("\rextracted %d/%d", completed, total);
	});
...
extern "C"
int launch_dyld_shared_cache_extractor_program(const char *shared_cache, const char *library_candidate, const char *output_path) {
	...
	int result = extract_shared_cache(library_path, shared_cache, output_path);

	printf("Finish extraction of shared cache\n");
	return result;
}